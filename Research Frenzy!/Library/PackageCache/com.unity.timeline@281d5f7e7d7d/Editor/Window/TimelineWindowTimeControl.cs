    ...
    class TimelineWindowTimeControl : IAnimationWindowControl
    {
        ...
        public override AnimationKeyTime time
        {
            get
            {
                ...
                return AnimationKeyTime.Time(ToAnimationClipTime(state.editSequence.time), m_AnimWindowState.frameRate);
            }
        }
        ...
        void ChangeTime(double newTime)
        {
            ...
            {
                // avoid rounding errors. This is in global time, so it's using state.referenceSequence.frameRate
        ...
        void ChangeFrame(int frame)
        {
            ...
            double frameTime = TimeUtility.FromFrames(frame, m_AnimWindowState.frameRate);
        ...
        {
            var newTime = AnimationWindowUtility.GetPreviousKeyframeTime(GetCurves(), time.time, m_AnimWindowState.frameRate);
            GoToTime(m_AnimWindowState.SnapToFrame(newTime, k_SnapMode));
        }

        public override void GoToNextKeyframe()
        {
            var newTime = AnimationWindowUtility.GetNextKeyframeTime(GetCurves(), time.time, m_AnimWindowState.frameRate);
            GoToTime(m_AnimWindowState.SnapToFrame(newTime, k_SnapMode));
        }
