...
namespace UnityEngine.Timeline
{
    public partial class TimelineAsset
    {
        ...
        public TrackAsset CreateTrack(Type type, TrackAsset parent, string name)
        {
            ...
            var tracks = parent != null ? parent.subTracksObjects : trackObjects;
            string trackName;
#if UNITY_EDITOR
            var baseName = string.IsNullOrEmpty(name) ? UnityEditor.ObjectNames.NicifyVariableName(type.Name) : name;
            trackName = TimelineCreateUtilities.GenerateUniqueActorName(tracks, baseName);
#else
            trackName = name ?? "";
#endif
